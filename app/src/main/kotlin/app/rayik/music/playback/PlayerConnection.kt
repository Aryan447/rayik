...
import app.rayik.music.extensions.getCurrentQueueIndex
import app.rayik.music.extensions.getQueueWindows
import app.rayik.music.models.MediaMetadata
import app.rayik.music.playback.MusicService.MusicBinder
...
class PlayerConnection(
    ...
    fun seekToNext() {
        val state = service.togetherSessionState.value as? app.rayik.music.together.TogetherSessionState.Joined
        if (state?.role is app.rayik.music.together.TogetherRole.Guest) {
            service.requestTogetherControl(app.rayik.music.together.ControlAction.SkipNext)
            return
        }
        player.seekToNext()
        player.prepare()
    ...
    fun seekToPrevious() {
        val state = service.togetherSessionState.value as? app.rayik.music.together.TogetherSessionState.Joined
        if (state?.role is app.rayik.music.together.TogetherRole.Guest) {
            service.requestTogetherControl(app.rayik.music.together.ControlAction.SkipPrevious)
            return
        }
